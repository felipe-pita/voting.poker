...
import { AvatarCTA } from "@/components/AvatarCTA";

const RootPaper = styled(Paper)<{ "data-is-dragging": boolean }>(
...
const RootBox = styled(Box)<{ "data-is-dragging": boolean }>(
	({ theme, "data-is-dragging": isDragging }) => ({
		display: "flex",
		flexDirection: "column",
		justifyContent: "center",
		...
		<>
			<Box>
				<Box display={"block"} position={"relative"}>
					<Avatar
						{...avatarProps(user.name, user.avatar, {
							width: 128,
							height: 128,
							fontSize: "4rem",
						})}
