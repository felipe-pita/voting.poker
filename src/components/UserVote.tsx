...
const ModeratorBadgeRoot = styled(Box)<{ "data-is-big": boolean }>(
	({ theme, "data-is-big": big }) => ({
...
const VoteBadgeRoot = styled(Box)<{
	"data-is-vote-empty": boolean;
	"data-is-big": boolean;
}>(({ theme, "data-is-vote-empty": emptyVote, "data-is-big": big }) => ({
...
const VoteBadge = ({
	...
	return (
		<VoteBadgeRoot data-is-vote-empty={!vote} data-is-big={variant === "big"}>
	...
	return (
		<Tooltip title={`${displayYouTag ? "(Você) " : ""}${name}`}>
			...
			>
				...
				>
					<Avatar
						{...avatarProps(name, avatar, { width: size, height: size })}
					/>
				</Badge>
			</Badge>
		</Tooltip>
...
const UserVoteButton = styled(Button)<{ "data-is-big"?: boolean }>(
	({ theme, "data-is-big": big }) => ({
		position: "relative",
...
const UserVoteBox = styled(Box)<{ "data-is-big"?: boolean }>(
	({ theme, "data-is-big": big }) => ({
		position: "relative",
		width: theme.spacing(big ? 5 : 8),
		height: theme.spacing(big ? 5 : 8),
